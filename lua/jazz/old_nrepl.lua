...
local parse_aliases = function(obj, file)
  ...
  local menu = impromptu.new.ask{
      ...
      handler = function(session, selected)
        ...
        if ix == "start" then
          nrepl.start(acid_utils.merge(config,
            {alias = aliases}))
  ...
  local command = function(fname)
    return ops.eval{code = '(run! println (keys (:aliases (read-string (slurp "' .. fname .. '")))))'}:with_handler(function(obj)
      if obj.out ~= nil then
  ...
  acid.run(command(os.getenv('HOME') .. "/.clojure/deps.edn"), conn)
  acid.run(command(obj.pwd .. "/" .. file), conn)
...
local toolsdeps = function(obj)
  local files = vim.fn.expand("**/*.edn", true, true)
